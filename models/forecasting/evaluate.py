...
# ── Load Model ────────────────────────────────────────────────────────
def load_best_model(checkpoint_dir: str = "models/checkpoints") -> TemporalFusionTransformer:
...
def generate_predictions(model, val_loader, val_df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"  ✓ {len(pred_df)} predictions | pred_p50 range: {p50.min():.5f} to {p50.max():.5f}")
    logger.info(f"  ✓ actual range: {pred_df['actual'].min():.5f} to {pred_df['actual'].max():.5f}")
    return pred_df
...
def generate_signals(pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    long_mask  = (pred_df["pred_p50"] >  threshold) & (pred_df["uncertainty"] < max_uncert)
    short_mask = (pred_df["pred_p50"] < -threshold) & (pred_df["uncertainty"] < max_uncert)

    pred_df.loc[long_mask,  "signal"] =  1.0
    pred_df.loc[short_mask, "signal"] = -1.0

    confidence = pred_df["pred_p50"].abs() / (pred_df["uncertainty"] + 1e-6)
    confidence = confidence.clip(0, 3) / 3
    pred_df["position_size"] = pred_df["signal"] * confidence

    logger.info(f"  ✓ Signals: {long_mask.sum()} long | {short_mask.sum()} short | {(pred_df['signal']==0).sum()} flat")
    return pred_df
...
def run_backtest(pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["gross_return"]     = daily["avg_signal"] * daily["avg_actual"]
...
def build_benchmarks(pred_df: pd.DataFrame) -> dict:
    ...
    spy = pred_df[pred_df["ticker"] == "9"].groupby("date")["actual"].mean().sort_index()
    if spy.empty:
        spy = ew
...
def compute_metrics(returns: pd.Series, name: str) -> dict:
    ...
    if len(returns) < 5:
        return {"name": name, "total_return": 0, "annual_return": 0,
                "annual_vol": 0, "sharpe_ratio": 0, "sortino_ratio": 0,
                "max_drawdown": 0, "calmar_ratio": 0, "hit_rate": 0}
    ...
    down_vol      = float(downside.std()) * np.sqrt(TRADING_DAYS) if len(downside) > 1 else 1e-6
    daily_rf      = RISK_FREE_RATE / TRADING_DAYS
    sharpe        = float((returns - daily_rf).mean() / daily_vol * np.sqrt(TRADING_DAYS)) if daily_vol > 0 else 0
...
def create_backtest_charts(strategy_df, benchmarks, all_metrics):
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=["Portfolio Value", "Drawdown", "Daily Returns Distribution", "Sharpe Comparison"],
    ...
    # Portfolio value
    fig.add_trace(go.Scatter(x=dates, y=strategy_df["portfolio_value"],
        name="AlphaFlow TFT", line=dict(color="#00e5ff", width=2)), row=1, col=1)
    colors = ["#ffd700", "#7b61ff", "#ff4444"]
    for (bname, bseries), color in zip(benchmarks.items(), colors):
        fig.add_trace(go.Scatter(x=bseries.index, y=bseries.values,
            name=bname, line=dict(color=color, dash="dash", width=1.5)), row=1, col=1)

    # Drawdown
    cum = (1 + strategy_df["net_return"]).cumprod()
    dd  = (cum / cum.cummax() - 1) * 100
    fig.add_trace(go.Scatter(x=dates, y=dd, fill="tozeroy",
        name="Drawdown", line=dict(color="#ff4444")), row=1, col=2)

    # Return distribution
    fig.add_trace(go.Histogram(x=strategy_df["net_return"] * 100,
        nbinsx=40, name="Returns", marker_color="#00ff9d"), row=2, col=1)

    # Sharpe comparison
    names   = [m["name"] for m in all_metrics]
    sharpes = [m["sharpe_ratio"] for m in all_metrics]
    bar_col = ["#00e5ff" if n == "AlphaFlow TFT" else "#555" for n in names]
    fig.add_trace(go.Bar(x=names, y=sharpes, marker_color=bar_col, name="Sharpe"), row=2, col=2)

    fig.update_layout(
        title="AlphaFlow — Backtest Report",
        paper_bgcolor="#070a0f", plot_bgcolor="#0d1117",
        font=dict(color="#c9d1d9"), height=800,
    )
...
def print_metrics_table(all_metrics):
    ...
    print("=" * 78)
    print(f"{'Strategy':<20} {'Return%':>8} {'Annual%':>8} {'Sharpe':>8} {'Sortino':>8} {'MaxDD%':>8} {'Hit%':>7}")
    print("-" * 78)
    for m in all_metrics:
        marker = " ◀" if m["name"] == "AlphaFlow TFT" else ""
        print(f"{m['name']:<20} {m['total_return']:>7.1f}% {m['annual_return']:>7.1f}% "
